...
public class AxisSetup16Controller implements EventHandler<KeyEvent>,TreeEventListener,RealTimeEventListener {
	...
	private void initialize() {
		...
		this.fxResolutionComboBox.setItems(CommandConst.RESOULTION_LIST);
		this.fxRecordingTimeLabel.setText("1cnt = 1ms");
		...
		if(NetworkServerManager.checkNetworkConnection() && NetworkServerManager.checkSocket()) {
	...
	private void handleAbsolute1() {
		...
		//			if(speed!=null) NetworkServerManager.handleAbsoluteRight(his.numberDriveNo, ptp, speed);
		//		}
		this.ptpAbsolute1.setText(StringUtil.addComma(this.ptpAbsolute1.getText()));
		String ptp = StringUtil.removeCommna(this.ptpAbsolute1.getText());
		String speed = StringUtil.removeCommna(this.fxSpeed.getText());
	...
	private void handleJoggingLeft() {
		System.out.println("handleJoggingLeft");
		...
		if(this.runHeldCheckbox.isSelected()) {
			...
			System.out.println("handleJoggingLeft isSelected NO");
			String speed = StringUtil.removeCommna(this.fxSpeed.getText());
			if(speed!=null)  NetworkServerManager.handleJoggingLeft(numberDriveNo, speed);
		}
	}
	...
	private void handleJoggingRight() {
		System.out.println("handleJoggingRight");
		if(this.runHeldCheckbox.isSelected()) {
			if(this.isRunHeld) {
				NetworkServerManager.stop(this.numberDriveNo);
				this.isRunHeld = false;
			...
			System.out.println("handleJoggingRight isSelected NO");
			String speed = StringUtil.removeCommna(this.fxSpeed.getText());
			if(speed!=null)  NetworkServerManager.handleJoggingRight(numberDriveNo, speed);
		}
	}
	...
	@FXML
	void onCheckRunHeld() {
		System.out.println("onCheckRunHeld");
	}
	...
	public void handleMotionSpeed() {
		...
		//변경된 speed 값을 다시 불러옴...
		this.fxSpeedLabel.setText(StringUtil.getNumnberFommat(NetworkServerManager.getInitialSpeed(this.numberDriveNo)));
	}
	...
	public void handleJogAcceleration() {
		...
		//변경된 Acceleration 값을 다시 불러옴...		
		this.fxAccLabel.setText(StringUtil.getNumnberFommat(NetworkServerManager.getInitialAcceleration(this.numberDriveNo)));
	}
    ...
    void onClickResolution(ActionEvent event) {
    	...
    	this.recordingTime = 10*time;
    	this.fxRecordingTimeLabel.setText("1cnt = "+this.fxResolutionComboBox.getSelectionModel().getSelectedItem()+"ms");
    }
...
//			Process process = runtime.exec("/home/jscs1/justek_motionware/etherCAT_data/getData.sh");
	...
	public void setPLC() {
		...
		{
			ErrorLogManager.getInstance().addErrorLog(Tag,"setPLC",e);
		}
	}
...
//		System.out.println("Now Status == "+binary);
	...
	public void handle(KeyEvent event) {
		...
		if(event.getCode()==KeyCode.ENTER && event.getSource().equals(this.tfTimeInterval)) {
			...
			//minimum time =100ms
			if(time<100) {
				System.out.println("Change request Time 100이하 입니다...");
				return;
			}
	...
	System.out.println(Tag + " :: DriverNo =" + Source.DriverNo);
