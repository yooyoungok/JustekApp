...
public class multiAxisComponent implements NodeRealDataUpdateListener {
	...
	@FXML
	public void onClieckToggleRightJog(ActionEvent event){
	    ToggleButton btnSelected = (ToggleButton) event.getSource();
	    if(btnSelected.isSelected()) {
	    	System.out.println("onClieckToggleRightJog");
	    	  this.handleJogRight();
	    }
	    else{
	...
	@FXML
	public void onClieckToggleLeftJog(ActionEvent event){
	    ToggleButton btnSelected = (ToggleButton) event.getSource();
	    if(btnSelected.isSelected()) {
	    	System.out.println("onClieckToggleLeftJog");
	    	  this.handleJogLeft();
	    }
	    else{
	    	this.onClickStop(event);
	...
	void handleJogLeft() {
		...
		if(this.runHeldCheck.isSelected()) {
			...
			System.out.println("handleJogLeft isSelected NO");
			String speed = StringUtil.removeCommna(this.fxSpeedField.getText());
			if(speed!=null)  NetworkServerManager.handleJoggingLeft(driverNo, speed);
		}
	}
	...
	@FXML
	void handleJogStop() {
		if(this.runHeldCheck.isSelected()) return;
		String driverNo = String.valueOf(driverBox.getSelectionModel().getSelectedIndex()+1);
	...
	public void handleMotionSpeed() {
		...
		String speed = NetworkServerManager.getInitialSpeed(DriveNo);
		if(speed!=null) {
	...
	public void handleJogAcceleration() {
		...
		String Acc = NetworkServerManager.getInitialAcceleration(DriveNo);
		if(Acc!=null) {
