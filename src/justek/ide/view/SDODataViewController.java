...
public class SDODataViewController implements TreeEventListener{
	...
	static final String Tag = "SDODataViewController";
    ...
    @FXML
    private TextArea fxCommentTextArea;
	...
	private void uploadData() {
		...
		//선택된 Motor가 설정되어 있지 않으면 리턴한다.
		if(this.mororName==null) return;
		...
		fxCommentTextArea.clear();
		if(DummyData.getInstance().getCommandArray().size()==0) return;
		
		for(String value : DummyData.getInstance().getCommandArray()) {
			this.fxCommentTextArea.appendText(value+"\n");
			this.fxCommentTextArea.positionCaret(0);
		}
	}

	private ObservableList<SDOInfo> setCurrent() {
	...
	private ObservableList<SDOInfo> setMotor() {
	...
	private ObservableList<SDOInfo> setAmp() {
	...
	private ObservableList<SDOInfo> setPosition() {
		...
		flag = true;
		info_pos.setName("Position Loop Proportional Gain");
	...
	private String getSDOString(String param) {
		String result = "Fail";
	...
	private void setSDOString(String param) {
	...
	private void saveDownloadAmp(ObservableList<SDOInfo> items) {
	...
	private void saveDownloadCurrent(ObservableList<SDOInfo> items) {
	...
	private void saveDownloadPosition(ObservableList<SDOInfo> items) {
	...
	private void saveDownloadMotor(ObservableList<SDOInfo> items) {
	...
	private void saveModifyData(String tapId, ObservableList<SDOInfo> items) {
		try {
			...
			if(file.exists()) {
				...
				String[] comments = this.fxCommentTextArea.getText().split("\n");
	 ...
	 * @since 2018. 8. 14.
	 */
	private void saveAsData(String fileName) {
		try {
			File file = new File("./home/motor/"+fileName);
			ObservableList<SDOInfo> items = this.totalTableData;
			ObservableList<String> command = DummyData.getInstance().getCommandArray();
			BufferedWriter out;
			System.out.println("저장함...=="+fileName);
			out = new BufferedWriter(new FileWriter(file));

//			for(String commandValue:command) {
//				out.write(commandValue);
//				out.newLine();
//			}
			...
			if(file.exists()) {
				boolean result = DialogManager.getInstance().showConfirmDialogWithResult(file+" 저장완료");
				if(result) {
					this.setMotorList();
				}
			}
	...
	private void saveFileChooser() {
		...
		fileChooser.setInitialDirectory(new File("./home/motor"));
		File file = fileChooser.showSaveDialog(this.mainApp.getPrimaryStage());

		if(file!=null) {
			System.out.println(Tag+ " Save Success == "+file.getAbsolutePath());
			this.saveAsData(file.getName());
		}
	}
