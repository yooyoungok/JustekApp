 ...
 * 2018.06.14  유영옥    신규생성                                     
 *------------------------------------------------------------------------------
 * 2018.09.04  유영옥    탭에 선택 및 추가시 실시간정보를 사용하지 화면일 경우 실시간 listener를 null처리한다.                    
 *------------------------------------------------------------------------------
 */
...
public class RootLayoutController implements RealTimeEventListener {
	...
	@FXML
	private ComboBox<String> fxIPComboBox; //사용자가 서버의 IP를 선택하도록 한다.

	@FXML
	private Label fxSerboModeLabel; //서버상태를 표시한다.
	...
	/**
	 * SystemConfigTab의 선택이 종료될때 시스템설정 모드를 종료할 것인지 확인후에 탭의 선택을 변경한다.
	 * @author : YOO YOUNGOK 
				...
				new ChangeListener<Tab>() {
					...
					public void changed(ObservableValue<? extends Tab> ov, Tab t, Tab t1) {
						...
						if(t1!=null && realTimeTabMap.containsKey(t1.getId())){
							System.out.println("실시간정보가 필요하다 : Tab Listener select changed == tabId == "+t1.getId());
						...
						//실시간정보를 사요하지 않으면 listener를 null처리한다. 2018.09.04
						else if(t!=null && t1!=null && !realTimeTabMap.containsKey(t1.getId())) {
							System.out.println("실시간정보가 필요없다 : Tab Listener select changed == tabId == "+t1.getId());
...
//			output.append("IOException : " + e.getMessage());	
	 ...
	 * @return  void
	 * @exception 
	 * @desc 사용자가 원하는 Servo IP를 설정하도록 한다.
	 * @since 2018. 8. 14.
	 */
	@FXML
	...
	/******************************
	 * 초기 system info tree의 값을 설정한다. -> 추후 삭제가능함..
	 * *******************************/
	...
	public ObservableList<TreeItem<String>> getNodeInfo() {
		...
		try {
			...
			CommandConst.DRIVER_MAP.clear(); // HashMap저장으로 수정중... 
			...
			while ((szLine = br.readLine()) != null) {
				...
				//져스텍 모터가 아닌경우는 우선 드라이버리스트에서 제외한다. => 추후 수정여부 확인할것...
