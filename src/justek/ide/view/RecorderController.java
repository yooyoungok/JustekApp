...
//		this.series2ComboBox.setValue(sourceList.get(5));
	...
	public void startLongTermTrigger() {
		...
		this.timerId = timer.addTimer(new TimerCall(1000)); //화면에 현재 소요시간을 체크하기 위한 timer실행
		this.fxTimeLabel.setText("Trigger 소요시간 : 0 시간 0 분 0초");
		GraphTriggerData.getInstance().setTriggerInfo(this.info, this.selectDriveNo,this);
		GraphTriggerData.getInstance().runLogProcess();

		GraphTriggerData.getInstance().checkFile(); // 과거 데이터가 있는지 확인한다.
	}
...
//    		mode = this.normalRadioButton.getText();
    	...
    	this.fxCountLabel.setText("1cnt="+info.getResolution()+"ms");
	...
	private void checkTriggerData() {
		...
		boolean result = GraphTriggerData.getInstance().saveData(CommandConst.DRIVER_NUMBEER);
		if(result) {
			boolean checkResult = GraphTriggerData.getInstance().checkTrigger();
			if(!checkResult) {
				DialogManager.getInstance().showServerErrorConfirmDialog("TriggerData가 없습니다.");
			}	
		}
	}
...
//		this.startTrigger();
	...
	private void recordingStop2() {
		...
		if(this.ignoreStart) {
			...
			GraphTriggerData.getInstance().stopLogProcess();
		}
	}
